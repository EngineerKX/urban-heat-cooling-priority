...
def class_mean_feature_vectors(X, valid_mask=None, feature_bands=ALL_FEATURE_BANDS,
    ...
    """`X`: (n_patches, H, W, len(feature_bands)+n_landcover_classes),
    channels-last (matches `cnn_data.build_local_feature_target_patches`).
    ...
    spectral = X[..., :n_bands]  # (n, H, W, n_bands)
    onehot = X[..., n_bands:n_bands + n_landcover_classes]  # (n, H, W, n_landcover_classes)
...
def apply_patch_counterfactual(patch_X, edit_mask, target_class: int, class_mean_vectors: dict,
    ...
    channels-last. `edit_mask`: (H, W) bool, pixels to convert to
