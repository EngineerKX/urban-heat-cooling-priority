#!/usr/bin/env python
"""Build the soft-voting RF+U-Net land-cover hybrid raster.
...
averages them, and argmaxes to a single full-Singapore hybrid raster --
the "Hybrid ML" deliverable named in the project proposal. Called a
*hybrid* (RF + CNN, two different model types) rather than an *ensemble*
(which combines multiple instances of the same model type).

Usage: python scripts/build_landcover_hybrid.py [--force]
"""
...
from src.landcover.hybrid import HYBRID_RASTER_PATH, build_hybrid
...
def main(force: bool = False):
    ...
    label_path, prob_path = build_hybrid(RF_PROB_RASTER_PATH, UNET_PROB_RASTER_PATH)
    print(f"\nDone. Hybrid raster: {label_path}")
    return label_path
