...
tiny synthetic patch -- no GEE required (module itself has no TensorFlow
dependency, only run_patch_counterfactual's model argument does). Same
runnable-script + printed pass/fail style as the rest of tests/ (no pytest
in this repo).
...
def _make_synthetic_patch_batch():
    ...
    channels, channels-last (n, H, W, C) -- matches
    cnn_data.build_local_feature_target_patches's layout. (0,0) and (1,0)
    are vegetation; (0,1) and (1,1) are built_up. No bare/water pixels at
    all -- exercises the "no pixels for this class" branch too."""
...
def test_apply_patch_counterfactual_edits_only_masked_pixels():
    X = _make_synthetic_patch_batch()
    means = class_mean_feature_vectors(X, feature_bands=FEATURE_BANDS, n_landcover_classes=N_CLASSES)
    patch_X = X[0]  # (2, 2, 6), channels-last
